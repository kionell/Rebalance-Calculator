...
class Tap
{
  static #spaced_buff_factor = 0.10;
  static #timescale_count = 4;

  static #timescale_factors = [1.02, 1.02, 1.05, 1.15];

  static #decay_coeffs = Generate.linearSpaced(Tap.#timescale_count, 2.3, -2.8).map(x => Math.exp(x));
  ...
  {
    let strainHistory = [
      new Array(Tap.#timescale_count).fill(0),
      new Array(Tap.#timescale_count).fill(0)
    ];
    let currStrain = new Array(Tap.#timescale_count).fill(0);
    ...
    if (hitObjects.length >= 2) {
      ...
      for (let i = 2, len = hitObjects.length; i < len; ++i) {
        ...
        currStrain = currStrain.map((x, i) => x * Math.exp(-Tap.#decay_coeffs[i] * (currTime - prevTime) / clockRate));
        ...
        let spacedBuff = Tap.calculateSpacedness(distance) * Tap.#spaced_buff_factor;
        ...
        currStrain = currStrain.map((x, i) => x + Tap.#decay_coeffs[i] * strainAddition *
    ...
    let strainResult = new Array(Tap.#timescale_count);

    for (let j = 0; j < Tap.#decay_coeffs.length; ++j) {
      ...
      let k = 1 - 0.04 * Math.sqrt(Tap.#decay_coeffs[j]);

      for (let i = 0, len = hitObjects.length; i < len; ++i) {
        singleStrainResult += singleStrainHistory[i] * Math.pow(k, i);
      }

      strainResult[j] = singleStrainResult * (1 - k) * Tap.#timescale_factors[j];
    }
