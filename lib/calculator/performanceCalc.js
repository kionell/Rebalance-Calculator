...
class PerformanceCalculator
{
  ...
  calculate(difficulty, score)
  {
    ...
    let aimValue = this._computeAimValue(difficulty, score) || 0;
    let tapValue = this._computeTapValue(difficulty, score) || 0;
    let accuracyValue = this._computeAccuracyValue(difficulty, score) || 0;

    let totalValue = multiplier * Mean.powerMean2(
      [aimValue, tapValue, accuracyValue], total_value_exponent
    );
  ...
  _computeAimValue(difficulty, score)
  {
    ...
    let modifiedAcc = this._getModifiedAcc(score);
    ...
    let aimValue = this._tpToPP(tp * cheeseFactor);

    // penalize misses
    aimValue *= Math.pow(0.96, Math.max(this.effectiveMissCount - miss_count_leniency, 0));
  ...
  _computeTapValue(difficulty, score)
  {
    if (this.beatmap.hitObjects.length <= 1) {
      return 0;
    }
    ...
    let tapValue = this._tapSkillToPP(tapSkill);
    ...
    tapValue *= Math.pow(0.93, Math.max(this.effectiveMissCount - miss_count_leniency, 0));
  ...
  _computeAccuracyValue(difficulty, score)
  {
    ...
    accuracyValue *= Math.pow(0.96, Math.max(this.effectiveMissCount - miss_count_leniency, 0));
  ...
  _tpToPP(tp) 
  {
    return Math.pow(tp, skill_to_pp_exponent) * 0.118;
  }

  _tapSkillToPP(tapSkill)
  {
    return Math.pow(tapSkill, skill_to_pp_exponent) * 0.115;
  }
}
